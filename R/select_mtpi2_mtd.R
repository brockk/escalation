...
#' implemented by \code{\link{get_mtpi2}}, including any additional behaviours
...
#'
#' @references Guo, W., Wang, SJ., Yang, S., Lynn, H., Ji, Y. (2017). A Bayesian
#' Interval Dose-Finding Design Addressing Ockham's Razor: mTPI-2.
#' https://doi.org/10.1016/j.cct.2017.04.006
select_mtpi2_mtd <- function(parent_selector_factory,
  ...
  class(x) <- c('mtpi2_mtd_dose_selector_factory',
                'derived_dose_selector_factory',
                'selector_factory')
  return(x)
}

mtpi2_mtd_dose_selector <- function(parent_selector,
                                    when = c('finally', 'always'),
                                    target = NULL,
                                    exclusion_certainty,
                                    alpha, beta,
  ...
  if(is.null(target)) {
    ...
    if(is.null(target)) {
      stop(paste0("Target toxicity probability is required when selecting dose",
                  " by mTPI2's MTD algorithm. Could not fetch from parent."))
    }
  }
  ...
  class(l) = c('mtpi2_mtd_dose_selector',
...
#' @importFrom magrittr %>%
#' @export
fit.mtpi2_mtd_dose_selector_factory <- function(selector_factory, outcomes,
                                                ...) {
  ...
  do.call(mtpi2_mtd_dose_selector, args = args)
}
