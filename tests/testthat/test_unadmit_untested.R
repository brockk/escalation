...
test_that('unadmit_untested_selector does what it should.', {
  ...
  expect_equal(dose_admissible(fit1), c(TRUE, TRUE, FALSE, FALSE, FALSE))
  check_dose_selector_consistency(fit1)

  ## CRM skipping allowed
  model2 <- get_dfcrm(skeleton = skeleton, target = target) %>%
    unadmit_untested()
  fit2 <- model2 %>% fit('1NNN')

  expect_equal(dose_admissible(fit2), c(TRUE, FALSE, FALSE, TRUE, FALSE))
  check_dose_selector_consistency(fit2)

  # mTPI2 example
  model3 <- get_mtpi2(num_doses = length(skeleton), target = target,
                      epsilon1 = 0.05, epsilon2 = 0.05,
                      exclusion_certainty = 0.95) %>%
    unadmit_untested()
  fit3 <- model3 %>% fit('1NNN')

  expect_equal(dose_admissible(fit3), c(TRUE, TRUE, FALSE, FALSE, FALSE))
  check_dose_selector_consistency(fit3)

})


test_that('unadmit_untested_selector supports correct interface.', {
