...
These functions fetch model fitting code.
Largely, this is imported from existing R packages.
These approaches can then be augmented with extra behaviours to specialise the dose selection process.
...
These last two are implemented natively in `escalation`.
We look at each now.

### get_dfcrm
The continual reassessment method [@OQuigley1990] is implemented in the [`dfcrm`](https://CRAN.R-project.org/package=dfcrm) package by @dfcrm.
...
Both of these topics will be the topics of full vignettes, coming soon.
